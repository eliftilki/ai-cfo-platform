...
export async function POST(req: Request) {
  ...
  try {
    ...
    return Response.json(
      { message: 'Gecersiz giris istegi.' },
      { status: 400 }
    );
  }

  try {
    const backendResponse = await fetch(`${getAiCfoBackendBaseUrl()}/auth/login`, {
      method: 'POST',
      headers: {
...
function extractErrorMessage(payload: unknown) {
  ...
  const errorPayload = payload as LoginErrorPayload | null;
  ...
  return 'Giris basarisiz. Lutfen e-posta ve sifreni kontrol et.';
}
